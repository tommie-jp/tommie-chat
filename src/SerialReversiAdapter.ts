...
// 1 行単位でメッセージを送受信する。プロトコルは RUP v0.2:
// https://github.com/tommie-jp/reversi-uart-protocol を参照。
//
// v0.2 主な変更点:
//   - 応答プレフィックス +/- 導入 (POP3 流)
//   - PO mnemonic 廃止 → +PI に統合
//   - ER<NN> mnemonic 廃止 → -<NN> に統合
//   - X* 拡張ネームスペース (XA-XZ は CPU 製作者の自由領域、§5.1 違反扱いしない)
//   - 改行 CR+LF 必須 (TX 側のみブリッジで担保。RX 側は寛容に CR/LF/CRLF 受理)
//
// Phase 5: PI/+PI ハートビート（3 秒間隔）、3 連続失敗で CPU 停止扱い。
...
const PING_INTERVAL_MS = 1000; // §6 サーバー側は 1000 ms 待機
const PING_FAIL_THRESHOLD = 3; // §6 3 連続失敗で切断扱い
const STUCK_THRESHOLD_MS = 10_000;     // PI/+PI 以外の活動が N ms 無ければ STUCK
...
export class SerialReversiAdapter {
    // §11 C1 (IDLE) / C2 (MY_TURN) / C3 (WAIT_OPP) のミラー
    ...
    // Phase 6: §13 復帰のため、直前に送った「ゲーム指示」を保持
    //          PI / VE / 応答系 (+/-) は対象外
    private lastDirectiveSent = "";

    // STUCK 検知: PI/+PI 以外の TX/RX があった最終時刻と、最後にダンプした時刻
    ...
    // §5.1 / §7.3: 仕様違反受信時に -<NN>[ <reason>] を返す (RUP v0.2)。
    // code は §7.3 応答コード表の 2 桁数値 (0-99)。reason はデバッグ用の任意文字列。
    // 未接続ならサイレント。
    private sendErrorResponse(code: number, reason?: string): void {
        const b = window.__serialTestBridge;
        if (!b || !b.isConnected()) return;
        const cc = String(code).padStart(2, "0");
        const payload = reason ? `-${cc} ${reason}` : `-${cc}`;
        b.sendLine(payload).catch((e) => console.warn("SerialReversi: -NN 送信失敗:", e));
    }

    // 受信 1 行のパース (RUP v0.2 §5/§7)。
    //   行頭 '+' = 直前送信への成功応答
    //   行頭 '-' = 直前送信へのエラー応答 (-NN[ <reason>])
    //   それ以外 = CPU 主導の自発通知
    // 仕様違反受信は §5.1 に従い -<NN> で応答する。
    // ブリッジ側 (adapterFeed) で改行 (CR/LF/CRLF) は除去済み前提。
    private onLine(raw: string): void {
        const line = raw.replace(/[\r\n]+$/g, "");
        if (line.length === 0) return; // 空行はサイレント破棄 (§5.1)

        // 成功応答 +<CMD>...
        if (line.startsWith("+")) {
            this.handleSuccessResponse(line.slice(1));
            return;
        }
        // エラー応答 -<NN>[ <reason>]
        if (line.startsWith("-")) {
            this.handleErrorResponse(line.slice(1));
            return;
        }

        const head = line.slice(0, 2);
        const rest = line.slice(2);
        // §5 コマンドは大文字 2 文字 → -02 (lowercase) もしくは -01 (unknown)
        if (!/^[A-Z]{2}$/.test(head)) {
            console.warn(`SerialReversi: lowercase or non-alpha command (§5 violation): ${JSON.stringify(head)}`);
            this.sendErrorResponse(2, `lowercase cmd ${head}`);
            this.emitStatus(false, `non-uppercase cmd: ${head}`);
            return;
        }
        // §5 拡張ネームスペース X* (1 文字目が X) は仕様違反扱いしない。ログ記録のみ。
        if (head.startsWith("X")) {
            console.log(`SerialReversi <CPU: ${head}${rest} (§5 X* extension, log only)`);
            this.emitStatus(true);
            return;
        }
        // STUCK 検知用: PI/+PI 以外の受信は「ゲーム活動」として記録
        if (head !== "PI") {
        ...
        switch (head) {
            case "MO":
                ...
                return;
            case "PA":
                // §12 MY_TURN でのみ意味がある。Nakama 側にパス RPC が無いため警告のみ
            ...
            case "RE":
                ...
                return;
            case "ST":
                // §7.2A #7 ST は自由形式テキスト。以前は診断用に "ST BO<64>" 形式で盤面スナップショットも
                // 流していたが、BS (§7.2A #6) に分離。後方互換のため当面は ST BO も受理する。
                if (/^\s*BO[0-2]{64}$/.test(rest)) {
                    console.warn(
                        `SerialReversi <CPU: "ST BO<64>" はレガシー形式 (現行プロトコルでは "BS<64>" を使用、§7.2A #6)`,
                    );
                }
                ...
                return;
            case "BS":
                // §7.2A #6 BOARD STATUS: CPU が自盤面スナップショットを報告。診断用。
                if (/^[0-2]{64}$/.test(rest)) {
                    console.log(`SerialReversi <CPU: BS ${rest}`);
                    this.emitStatus(true);
                } else {
                    console.warn(`SerialReversi <CPU: BS 不正書式 (§7.2A #6) → -04`);
            ...
            case "NC":
                ...
                return;
            default:
                // §5.1: 未知コマンドは仕様違反 → -01 で応答
                console.warn(`SerialReversi <CPU: 未知コマンド "${line}" (§5.1 violation) → -01`);
    ...
    // §7.2B 成功応答 +<CMD>... (head の '+' は除去済み)
    private handleSuccessResponse(body: string): void {
        ...
        // +CA <space-separated mnemonic>: CAPABILITY 応答 (任意実装、ログのみ)
        if (head === "CA") {
            console.log(`SerialReversi <CPU: +CA${rest}`);
            this.emitStatus(true);
            return;
        }
        // +X* 拡張クエリへの応答 (§5/§7.2B #12)
        if (head.startsWith("X")) {
            console.log(`SerialReversi <CPU: +${head}${rest} (§5 X* extension response)`);
            this.emitStatus(true);
            return;
        }
        // +<CMD> 通知への任意 ack (§7.2B #13) はサイレント破棄
        console.debug(`SerialReversi <CPU: +${body} (notification ack, dropped)`);
    }

    // §7.2C エラー応答 -<NN>[ <reason>] (head の '-' は除去済み)
    // ログ記録のみ、自動再送はしない (§5.1)。
    private handleErrorResponse(body: string): void {
        const m = body.match(/^(\d{2})(?:\s+(.*))?$/);
        if (m) {
            const code = m[1];
            const reason = m[2] ?? "";
            console.log(`SerialReversi <CPU: -${code}${reason ? ` reason="${reason}"` : ""}`);
        } else {
            console.warn(`SerialReversi <CPU: - (malformed error response): ${body}`);
        }
        this.emitStatus(true);
    }
    ...
    private handleMoMessage(rest: string): void {
        // §5 / §7.1 / §8: 座標は小文字のみ受理。MOD3 等の大文字は -04 で拒否
        const m = rest.match(/^([a-h])([1-8])/);
        if (!m) {
            console.warn(`SerialReversi <CPU: MO${rest} 不正座標 (§8 violation) → -04`);
    ...
    // CPU からの RS (REQUEST SYNC §7.2A #5)。盤面乖離検知時の再同期要求。
    // §13.1 に従い、現在のサーバ盤面を BO で送り、直前の指示 (通常 MO?) を再送する。
    ...
    // +PI 受信: ハートビートリセット (RUP v0.2 では PO は廃止、+PI に統合)
    private onPongReceived(): void {
        if (this.cpuStalled) {
            console.log("SerialReversi <CPU: +PI 受信 - CPU 復活");
        }
    ...
    // 1 秒おきに PI を送る。未接続時はスキップ。3 連続 +PI 失敗で停止フラグ。
    private pingTick(): void {
        ...
        if (this.pingOutstanding >= PING_FAIL_THRESHOLD && !this.cpuStalled) {
            ...
            console.warn(`SerialReversi: CPU 応答なし (+PI 連続 ${PING_FAIL_THRESHOLD} 回失敗) - MO 送信を停止`);
        }
    ...
    // sendDirective は常にゲーム活動 (PI/VE/+/- は sendDirective 経由で送らない)
...
// §8 初期局面: d4=W(2), e4=B(1), d5=B(1), e5=W(2)
...
// §8 / §7.1 #5 BO の並び順: 行優先 (a1..h1, a2..h2, ... h8) の 64 文字
