...
- 第7回 自作CPUを語る会 [doc/58-自作CPUオセロ大会ルール.md](doc/58-自作CPUオセロ大会ルール.md) もこの形式。
...
[public/serial-test.html](../public/serial-test.html) に Web Serial API の最小テスト
ページを用意済み。Vite 経由で配信されるので `/serial-test.html` でアクセス可能。

機能:

- ボーレート選択（9600 / 38400 / 115200）
- Connect / Disconnect
- 受信データのリアルタイム表示
- 任意文字列の送信（`\r\n` 自動付加）
...
```bash
npm run dev
# → http://localhost:3000/serial-test.html を Chrome で開く
# → Connect → ポート選択 → デバイスのログが流れれば成功
```
...
`http://192.168.x.x:3000/serial-test.html` は secure context にならないため、
...
4. `http://192.168.x.x:3000/serial-test.html` を開く → Connect
...
→ `npm run dev` → `https://192.168.x.x:3000/serial-test.html`
...
ステージングに `serial-test.html` を含むビルドをデプロイすれば、
...
- 大会ルール: [doc/58-自作CPUオセロ大会ルール.md](doc/58-自作CPUオセロ大会ルール.md)
- オセロ実装: [doc/56-設計-対戦オセロ.md](doc/56-設計-対戦オセロ.md)
- Web Serial API:
