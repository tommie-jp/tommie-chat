...
$COMPOSE exec -T postgres psql -U nakama -d nakama -c \
    "SELECT regexp_replace(username, '[0-9_]+$', '*') AS pattern, count(*) AS count, min(create_time)::date AS created FROM users GROUP BY pattern ORDER BY count DESC LIMIT 20;"

# 手動作成のユーザー（テスト以外）
echo "  手動ユーザー:"
$COMPOSE exec -T postgres psql -U nakama -d nakama -c \
    "SELECT u.username, d.id AS device_id, u.create_time::date AS created FROM users u LEFT JOIN user_device d ON u.id = d.user_id WHERE u.username NOT LIKE '%\_%' ORDER BY u.username;"
echo ""
...
$COMPOSE exec -T postgres psql -U nakama -d nakama -c "
    ...
    DELETE FROM storage WHERE user_id != '00000000-0000-0000-0000-000000000000';
    DELETE FROM users;
"
