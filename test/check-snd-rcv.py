...
LARGE_MATCH_TOLERANCE = 0.99  # 大人数時: この割合以上マッチなら警告扱い（ログ欠損許容）
...
PAIRS_MOVETARGET = [
    ('snd moveTarget',      'rcv moveTarget',      'moveTarget(C→S)',   'cs'),
    ('rcv matchdata op=2',  'snd moveTarget',      'moveTarget(S→C)',   'sc'),
]
# --avatarchange: opAvatarChange テスト
PAIRS_AVATARCHANGE = [
    ('snd avatarChange',    'rcv avatarChange',    'avatarChange(C→S)', 'cs'),
    ('rcv matchdata op=3',  'snd avatarChange',    'avatarChange(S→C)', 'sc'),
]
...
def check_pair(
    ...
    if unmatched:
        # 大人数でマッチ率が十分高い場合はログ欠損として警告扱い
        match_rate = matched / len(first) if len(first) > 0 else 0
        tolerated = is_large and match_rate >= LARGE_MATCH_TOLERANCE and not warn_only
        if tolerated:
            icon = '⚠️'
            print(f'  {icon} {label}  ログ欠損 {len(unmatched)}件（{match_rate*100:.1f}%マッチ — 許容）'
                  f' (matched={matched}/{len(first)}, client={len(c_evs)}, server={len(s_evs)},'
                  f' window=[-{CLOCK_SLACK:.1f}s,+{win:.1f}s])')
            return True, False  # ok=True: エラーカウントしない
        icon = '⚠️' if warn_only else '❌'
...
def main() -> int:
    ...
    parser.add_argument('--setblock',    action='store_true',
                        help='setBlock/getGroundChunk チェックも実施（setBlock テスト用）')
    parser.add_argument('--movetarget',  action='store_true',
                        help='moveTarget C→S/S→C チェックも実施（opMoveTarget テスト用）')
    parser.add_argument('--avatarchange', action='store_true',
                        help='avatarChange C→S/S→C チェックも実施（opAvatarChange テスト用）')
