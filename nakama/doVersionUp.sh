#!/bin/bash
# index.html と package.json のバージョン・日付を更新するスクリプト
# Usage: ./nakama/doVersion.sh <new-version>
#   例: ./nakama/doVersion.sh 0.1.21
#
# 更新対象:
#   index.html   — APP_VERSION, APP_DATE
...
show_help() {
    ...
    echo ""
    echo "更新対象:"
    echo "  index.html   — APP_VERSION, APP_DATE (今日の日付)"
...
if [ $# -eq 0 ]; then
    # 引数なし: index.html の APP_VERSION からマイナーバージョンを +1
...
# ---------- 現在の値を表示 ----------
OLD_HTML_VER=$(grep -oP 'APP_VERSION\s*=\s*"\K[^"]+' index.html)
OLD_HTML_DATE=$(grep -oP 'APP_DATE\s*=\s*"\K[^"]+' index.html)
OLD_PKG_VER=$(grep -oP '"version"\s*:\s*"\K[^"]+' package.json)

echo "=== バージョン更新 ==="
echo ""
echo "index.html:"
echo "  APP_VERSION: \"$OLD_HTML_VER\" → \"$NEW_VER\""
echo "  APP_DATE:    \"$OLD_HTML_DATE\" → \"$NEW_DATE\""
echo ""
echo "package.json:"
...
sed -i "s/var APP_VERSION = \"[^\"]*\"/var APP_VERSION = \"$NEW_VER\"/" index.html

# index.html — APP_DATE
sed -i "s|var APP_DATE    = \"[^\"]*\"|var APP_DATE    = \"$NEW_DATE\"|" index.html
...
echo "✅ 更新完了"
echo ""
grep -n 'APP_VERSION\|APP_DATE' index.html | head -2
